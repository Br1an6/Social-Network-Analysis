...
# In this project, we'll implement community detection and link prediction algorithms using Facebook "like" data.
#
    ...
    while num <= max_depth and queue:
        ...
        for adjacent in nei_list:
            if adjacent not in queue:
                if node == root or (adjacent not in node2parents[node] and adjacent not in poped):
                    queue.append(adjacent)
                    node2distances[adjacent] = num
                    node2num_paths[adjacent] = 1
                    node2parents[adjacent].append(node)
            else:
                if num <= node2distances[adjacent]:
                    node2num_paths[adjacent] += 1
                if node not in node2parents[adjacent] and node2distances[node] < node2distances[adjacent]:
                    node2parents[adjacent].append(node)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for key, val in node2distances.items():
        ...
        node_val_dict[key] = 1
    dist_list.sort(key=lambda tup: tup[1], reverse = True)

    for key, val in dist_list:
        if key != root:
            for p in node2parents[key]:
                if p in node_val_dict:
                    node_val_dict[p] += (node_val_dict[key] / len(node2parents[key]))
                else:
                    node_val_dict[p] = (node_val_dict[key] / len(node2parents[key]))
    ...
    for key, val in dist_list:
        if key != root:
            for p in node2parents[key]:
                ...
                ret_dict[tup_of_two_node] = node_val_dict[key]/len(node2parents[key])
...
def get_subgraph(graph, min_degree):
    ...
    """
    ###TODO  done
    degree_of_graph = nx.degree(graph)
    result = set([])

    for d in degree_of_graph:
        if degree_of_graph[d] >= min_degree:
            result.update(set([d]))
...
def volume(nodes, graph):
    ...
    re_edges = len(temp_graph.edges())

    ret_edges = all_edges - re_edges
    return ret_edges
...
def cut(S, T, graph):
    ...
    ret_edges = all_edges - s_edges - t_edges
    return ret_edges
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO done
    cut_of_graph = cut( S, T, graph )
    ret_No_Val = (float(cut_of_graph) / volume(S, graph)) + (float(cut_of_graph) / volume(T, graph))
    return ret_No_Val
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
        ...
        norm_val = norm_cut(components[0], components[1], graph)
...
def jaccard(graph, node, k):
    ...
    ###TODO done
    scores = []
    nei_list = set(graph.neighbors(node))
    for no in graph.nodes():
        if no != node and not graph.has_edge(node, no):
            nei_list2 = set(graph.neighbors(no))
            score = 1.* len(nei_list & nei_list2)/len(nei_list | nei_list2)
            scores.append(((node, no), score))

    scores.sort(key=lambda tup: tup[0])
    scores.sort(key=lambda tup: tup[1], reverse = True)
    ret_list = []
    for i in range(k):
        ret_list.append(scores[i])
...
def path_score(graph, root, k, beta):
    ...
    scores = []
    nodes_without_root_nei = set(graph.nodes()) - set(graph.neighbors(root))
...
def evaluate(predicted_edges, graph):
    ...
    ###TODO done
    corret = 0.
    for et in predicted_edges:
        if et[0] in graph.neighbors(et[1]):
            corret += 1.

    return corret / len(predicted_edges)
